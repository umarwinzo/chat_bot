...
Queen.addCommand({
  ...
  if (args.length === 0) {
    await sock.sendMessage(message.key.remoteJid, { text: Queen.errorMessage("Please provide a question for AI!") });
    return;
  }
  
  const question = args.join(' ');
  await sock.sendMessage(message.key.remoteJid, { text: Queen.infoMessage("🤖 AI is thinking...") });
  ...
  try {
    ...
    await sock.sendMessage(message.key.remoteJid, { text: Queen.errorMessage("AI service is currently unavailable!") });
  }
});
...
Queen.addCommand({
  ...
  await sock.sendMessage(message.key.remoteJid, { text: Queen.infoMessage("📰 Fetching latest news...") });
  ...
  try {
    ...
    await sock.sendMessage(message.key.remoteJid, { text: Queen.errorMessage("Failed to fetch news updates!") });
  }
});
